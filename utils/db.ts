...
export const insertDebt = (name: string, value: number, paid: boolean) => {
  db.runSync(
    'INSERT INTO debts (name, value, paid) VALUES (?, ?, ?)',
    [name, value, paid ? 1 : 0]
  );
};

export const insertIncome = (name: string, value: number, received: boolean) => {
  db.runSync(
    'INSERT INTO incomes (name, value, received) VALUES (?, ?, ?)',
    [name, value, received ? 1 : 0]
  );
};
...
export const updateIncomeReceived = (id: number, received: boolean) => {
  db.runSync(
    'UPDATE incomes SET received = ? WHERE id = ?',
    [received ? 1 : 0, id]
  );
};
