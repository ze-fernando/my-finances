...
function Home() {
  ...
  const loadData = async () => {
    ...
    const debtsPaidMap: Record<string, boolean> = debtsPaidState
      ? JSON.parse(debtsPaidState)
      : {};
    const incomesReceivedMap: Record<string, boolean> = incomesReceivedState
      ? JSON.parse(incomesReceivedState)
      : {};

    const mappedDebts = dbDebts.map((d) => ({
      ...d,
      paid: debtsPaidMap[d.id] ?? d.paid,
    }));
    const mappedIncomes = dbIncomes.map((i) => ({
      ...i,
      received: incomesReceivedMap[i.id] ?? i.received,
    }));
        ...
        >
          ...
          <Text
            style={{
              fontSize: 22,
              fontWeight: "bold",
              color: hideValues ? "white" : getPercentageColor(spentPercentage),
            }}
          >
            {hideValues
              ? "••••"
